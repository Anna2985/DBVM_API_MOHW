...
namespace ConsoleApp_stock
{
    class Program
    {
        ...
        {
            Console.Title = "ConsoleApp_stock";
            ...
            while (true)
            {
                foreach (var item in settingClasses)
                {
                    using (var conn_oracle = new OracleConnection(conn_str))
                    {
                        ...
                        // 2. 解析條碼 → CommandText
                        //===============================
                        string commandText = $"select * from THESE.MIS_STK_AMT WHERE DPT_NAME = '{item.設備名稱}' ";
                        ...
                        using (var reader = cmd.ExecuteReader(CommandBehavior.CloseConnection))
                        {
                            ...
                            while (true)
                            {
                                ...
                                try
                                {
                                    ...
                                    string 料號 = SafeGet(reader, "GOOD_CODE").Trim();

                                    medClass medclasses = dic_medclass.GetByCode(料號).Where(temp => temp.開檔狀態 == "開檔中").FirstOrDefault();

                                    if (medclasses == null)
                                    {
                                        //Logger.Log($"藥檔找無此料號{料號}");
                                        continue;
                                    }
                    ...
                    //Logger.Log(stockClasses.JsonSerializationt(true));
                    ...
                    List<stockClass> stock_buff = stockClasses
                        ...
                        .ToList();
                    //Logger.Log(stock_buff.JsonSerializationt(true));
                    ...
                    if (add.Count > 0)
                    {
                        ...
                        Console.WriteLine(returnData_add.Result);
                    ...
                    if (update.Count > 0)
                    {
                        ...
                        Console.WriteLine(returnData_update.Result);
